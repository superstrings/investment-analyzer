...
async def api_stock_detail(
    ...
    # Current position
    account_ids = [
        a.id for a in db.query(Account).filter_by(user_id=user_id).all()
    ]
    position = None
    if account_ids:
        latest_dates = (
...
async def api_search(
    ...
    from db.models import Account, Kline, Position, WatchlistItem
    ...
    # Search positions
    account_ids = [
        a.id for a in db.query(Account).filter_by(user_id=user.id).all()
    ]
    if account_ids:
        positions = (
            db.query(Position)
    ...
    if len(results) < 15:
        klines = (
            db.query(Kline.market, Kline.code)
            .filter(
                or_(
